...
def region_forward_dynamic(output, conf_thresh, num_classes, anchors, num_anchors, only_objectness=1,
                              validation=False):
...
class RegionLayer(nn.Module):
    ''' Region layer (Yolov2 yolo layer)
    model_out: while inference,is post-processing inside or outside the model
        true:outside
    '''
    def __init__(self, num_classes=0, anchors=[], num_anchors=1, stride=32, model_out=False):
    ...
    def forward(self, output, target=None):
        ...
        return region_forward_dynamic(output, self.thresh, self.num_classes, self.anchors, self.num_anchors)
