...
class Darknet(nn.Module):
    def __init__(self, cfgfile, inference=False, zaitzeff_mode=False):
        super(Darknet, self).__init__()
        self.inference = inference
        self.zaitzeff_mode = zaitzeff_mode
    ...
    def forward(self, x):
        ...
        if self.zaitzeff_mode or self.training:
            return out_boxes
        else:
    ...
    def create_network(self, blocks):
        ...
        for block in blocks:
            if block["type"] == "net":
                ...
                yolo_layer.zaitzeff_mode = self.zaitzeff_mode
